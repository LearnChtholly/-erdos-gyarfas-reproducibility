...
int main(int argc,char**argv){
 ...
 in>>tok>>kind>>nc; if(tok!="CERT_V1"||kind!="B15E1"||nc!=254){cerr<<"bad header\n";return 3;}
 ...
 for(int ci=0;ci<nc;ci++){
   ...
   for(int v=0;v<15;v++){expectedCap[v]=4+(v==sp)-deg[v];if(expectedCap[v]<0){cerr<<"negative cap\n";return 8;}if(expectedCap[v])labs.push_back(v);}
   if(expectedCap!=cap){cerr<<"cap mismatch case "<<ci<<"\n";return 9;}
   vector<Opt> opts;
   for(int i=0;i<EP;i++)for(int j=i+1;j<EP;j++){
     if(trigger(ed[i],ed[j],0,-1,-1,sp)&&local_safe(g,i,j,0,-1,-1))opts.push_back({i,j,0,-1,-1});
     for(int p:labs)if(cap[p]>=1)if(trigger(ed[i],ed[j],1,p,-1,sp)&&local_safe(g,i,j,1,p,-1))opts.push_back({i,j,1,p,-1});
     for(int p:labs)for(int q:labs){if(p==q&&cap[p]<2)continue;if(trigger(ed[i],ed[j],2,p,q,sp)&&local_safe(g,i,j,2,p,q))opts.push_back({i,j,2,p,q});}
   }
 ...
 cout<<"PASS\n";
 cout<<"cases=254 options="<<totalOpts<<" states="<<totalStates<<" branches="<<totalBranches<<"\n";
